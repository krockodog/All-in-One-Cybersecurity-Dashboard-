...
describe("cyber-data", () => {
  describe("toolCatalog", () => {
    ...
    it("should have valid categories", () => {
      const validCategories = ["osint", "pentest", "recon", "cloud", "forensics", "binary"];
      toolCatalog.forEach((tool) => {
  ...
  describe("toolsByCategory", () => {
    it("should have all six categories", () => {
      ...
      expect(toolsByCategory).toHaveProperty("forensics");
      expect(toolsByCategory).toHaveProperty("binary");
    });
    ...
    it("should have tools in each category", () => {
      ...
      expect(toolsByCategory.cloud.length).toBeGreaterThan(0);
      expect(toolsByCategory.forensics.length).toBeGreaterThan(0);
      expect(toolsByCategory.binary.length).toBeGreaterThan(0);
    });
    ...
    it("should filter tools correctly", () => {
      ...
      toolsByCategory.cloud.forEach((tool) => {
        expect(tool.category).toBe("cloud");
      });
      toolsByCategory.forensics.forEach((tool) => {
        expect(tool.category).toBe("forensics");
      });
      toolsByCategory.binary.forEach((tool) => {
        expect(tool.category).toBe("binary");
      });
    });
  });
  ...
  describe("categoryLabel", () => {
    it("should return correct labels", () => {
      ...
      expect(categoryLabel("cloud")).toBe("Cloud Security");
      expect(categoryLabel("forensics")).toBe("Forensics");
      expect(categoryLabel("binary")).toBe("Binary Analysis");
    });
  });
