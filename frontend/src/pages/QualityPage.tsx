    ...
    <section className="space-y-5" data-testid="quality-status-page">
      <header className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-slate-300" data-testid="quality-status-updated-at">
            Letztes Update: {payload?.updatedAt ?? "wird geladen..."}
          </p>
        </div>
        <button
      ...
      <div className="grid gap-3 md:grid-cols-5" data-testid="quality-metrics-grid">
        <div className="rounded-lg border border-white/10 p-3 text-sm" data-testid="quality-metric-targets">Targets: {payload?.metrics.targets ?? 0}</div>
        <div className="rounded-lg border border-white/10 p-3 text-sm" data-testid="quality-metric-pentests">Pentests: {payload?.metrics.pentests ?? 0}</div>
        <div className="rounded-lg border border-white/10 p-3 text-sm" data-testid="quality-metric-findings">Findings: {payload?.metrics.findings ?? 0}</div>
        <div className="rounded-lg border border-white/10 p-3 text-sm" data-testid="quality-metric-active">Aktiv: {payload?.metrics.activePentests ?? 0}</div>
        <div className="rounded-lg border border-white/10 p-3 text-sm" data-testid="quality-metric-critical">Kritisch: {payload?.metrics.criticalFindings ?? 0}</div>
      </div>
      ...
      <div className="rounded-xl border border-white/10" data-testid="quality-review-table">
        ...
        <div className="divide-y divide-white/10">
          {(payload?.reviewCycles ?? []).map((cycle) => (
